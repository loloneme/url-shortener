...
import (
	...
	"net/http/httptest"
	"testing"
	mock "url-shortener/internal/api/redirect/mocks"
...
func TestRedirectToOriginalSuccess(t *testing.T) {
	...
	err := handler.RedirectToOriginal(c)
...
func TestRedirectToOriginalFail(t *testing.T) {
	t.Run("invalid request", testBadRequest)
	t.Run("url not found", testNotFound)
